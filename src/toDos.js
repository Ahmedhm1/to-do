...
export default function ToDos() {
  ...
  let toDoList = useMemo(() => {
    if (toDo.length > 0) {
      ...
      return toDoToBeRender.map((task) => {
        return (
    ...
    <>
      <Container maxWidth="sm">
        <div className="box">
          ...
          <div className="add">
            <Button variant="contained" className="btnTitle" disabled={title ? false : true} style={{ backgroundColor: title ? "var(--primary-dark)" : "var(--primary-light)" }} onClick={() => title ? handleAddNewTask() : handleAddNewAlert("error", "يجب ان تملأ عنوان المهمة اولاً")}>إضافة</Button>
